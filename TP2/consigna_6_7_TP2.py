...
OPENWEATHER_API_KEY = "e966331c499b0cb292669902c4783d8a"
...
def api_capturar():
    ...
    cantidad = int(data.get("cantidad", 1))       # Cuántas mediciones quiere hacer
    intervalo = float(data.get("intervalo", 1.0)) # Segundos entre cada una
    ...
    try:
        # 1. Obtener clima una sola vez para ahorrar llamadas a la API
        if ciudad:
            clima = clima_por_ciudad(ciudad)
        else:
            clima = geo_latlon()
        ...
        # 2. Bucle para simular las múltiples capturas
        for i in range(cantidad):
            lectura = simular_lectura(lugar, altura, clima)
            lectura = insertar_lectura(lectura)
            lecturas_realizadas.append(lectura)
            
            # Si no es la última captura, esperamos el intervalo
            if i < cantidad - 1:
                time.sleep(intervalo)

        return jsonify({
            "mensaje": f"Se realizaron {cantidad} capturas correctamente",
            "lecturas": lecturas_realizadas
        }), 201
